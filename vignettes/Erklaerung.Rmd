...
Die Geomagnetikdaten wurden aus DLMGPS als .asc Datei exportiert.

Die folgenden Schritte wurden in SAGA durchgeführt, um die Daten in ein Rasterformat zu überführen
...
Nun sollten alle Anomalien einen Punkt haben. Diesen Layer speichern.

## Einladen der Daten
Die Daten sind vorbereitet vorhanden. Es handelt sich um den Ausschnitt aus einem Magnetikbild. Es handelt sich dabei um eine SAGA .sdat Datei, um die Messwerte nutzen zu können. Des weiteren ist eine Shapedatei vorhanden, die für jede Anomalie über 2nT einen Punkt enthält.
...
Im ersten Schritt bietet sich an, die Dipole zu markieren.
Dazu sind einige Parameter nötig

Was diese tun wird im laufe der Vignette erklärt. Zunächst nur, get_dipole = TRUE gibt an, dass Dipole ermittelt werden sollen
...
Als nächstes wird der suchradius zum Durchmesser, da dieser im Script weiterhin verwendung findet.
Außerdem heißt der SpatialDataFrame in der function  anomalies_sdf, in diesem Beispiel soll nur eine Anomalie verwendet werden
...
Für jede Anomalie soll eine vordefinierte Menge an Profilen erstellt werden, aus denen die Daten für den Test gewonnen werden.
Dazu muss ermittelt werden, wie viele anomalien vorhanden sind und wie die Auflösung des Rasters ist (die Raster haben die gleiche x und y Auflösung)
...
Jetzt wird eine Schleife durchlaufen, die jeden Anomaliepunkt abfrühstückt. Die Zählvariable ist k. Da die Schleife in diesem Beipiel keinen Sinn ergibt, wird sie auskommentiert
...
Aus der Auflösung und dem Suchradius lässt sich nun feststellen, wie viele Punkte das Profil haben muss. Damit der mittlere Punkt auch mittig ist, wird bei einer geraden Anzahl an Punkten einer hinzugefügt.
...
Dann wird der westlichste Punkt des Profiles - das erste Profil liegt immer Ost-West - ermittelt und von diesem immer ein Punkt hinzugefügt, mit dem Abstand der Rasterauflösung
...
Nun werden die Werte ausgelesen und verglichen. Da ein Dipol nicht immer komplett von einem negativen Bereich umgeben ist, sondern dieser auch nur in eine Richtung liegen kann. Muss dieser Vergleich in mehreren Schritten erfolgen. Dazu werden die Werte aus dem eben angelegten Profil ausgelsen und das Profil dann in **angle_steps** Grad gedreht und dann wieder ausgelesen. Erfüllt eines der Profile die Bedingungen eines Dipols, wird die Anomalie markiert.
...
Nun zu den zwei wichtigsten Schritten der Erkennung eines Dipoles.

Die Amplitude einer Anomalie sieht immer etwa so aus:
```{r, fig.width=7, fig.height=5}
      plot(mag_profile$x,mag_profile$value)
      abline (h=0,col="red")
```

Nach Vergleichen mehrere Datensätze lässt sich ein Dipol natürlich an Hand des Kurvenverlaufes definieren, aber auch anhand des maximalen und minimalen Wertes. Dazu ist der **dipol_factor** nötig. Denn der Dipol ist definiert durch |niedrigster nT-Wert x **dipole_factor**| > maximaler nT-Wert.
...
Dann wie oben Durchgeführt, Wiederholung der Prüfung des Profils. Dabei wird der Kurvenverlauf eines Dipols sichtbar.Und dieser auch als solcher erkannt, da der Betrag des Minimalwertes x **dipolfactor** (in diesem Fall = 2) größer ist als der Maximalwert. Die Klassifizierung erfolgt in der Spalte *di_kB* (0=kein Dipol; 1=Dipol; 2=Minimum < Grenzwert)
...
Um die Breiten und Höhen miteinander vergleichen zu können ist ein Wert nötig (**cut_value**) der als Grundlage für die Ermittlung der Werte genommen wird. In diesem Beispiel 2nT. D.h., dass auf der Höhe von 2 nT die Werte ermitteln werden. 

Zunächst definieren wir die Parameter, die nötig sind. Hier wird nur die Methode "avg" vorgestellt. Dabei wird die Breite jedes Profils um eine Anomalie berechnet und der Durchschnitt aller genommen. Bei "median" wird der median verwendet.
...
Die Daten dürfen keine NA Werte haben, dass kann passieren, liegt eine Anomalie am Rand des Bildes.
...
Jetzt muss noch die Höhe ausgelesen werden, dabei sollte es sich um den höchsten Wert des Profils handeln *möglicherweise testen, ob höchster Wert in der Mitte, da sonst Nachbaranomalie der Grund sein könnte*
